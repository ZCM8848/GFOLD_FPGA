...
module tb_drs_iter;
    ...
    reg [63:0] coo_word [0:2*NNZ-1];   // packed COO for SRAM preload
    integer k, i;
    reg watchdog = 0;
    always begin #(64'd300000000000); if (!watchdog) begin $display("TIMEOUT: sim stuck"); $finish; end end
    ...
    always @(posedge clk) begin
        if (dut.st !== prev_st_d && dut.st >= 0 && dut.st <= 300) begin
            if ($time >= 64'd95000000000 && $time <= 64'd104000000000) begin  // iter1: 91ms..104ms
    ...
    initial begin
        ...
        // ---- preload external SRAM: packed COO + initial band (LDL B array) ----
        ...
        for (k = 0; k < (HB+1)*N; k = k + 1) begin
            sram_mem[4*(dut.AA_BASE + k)+0] = band[k][15:0];
            sram_mem[4*(dut.AA_BASE + k)+1] = band[k][31:16];
            sram_mem[4*(dut.AA_BASE + k)+2] = band[k][47:32];
            sram_mem[4*(dut.AA_BASE + k)+3] = band[k][63:48];
        end
