`timescale 1ns/1ps
// Stage-B banded LDL^T solver in IEEE-754 FP64 (double). Same FSM as banded_ldl.v
// (float32) but with fp64_mul/add/div units and 64-bit band words. This is the
// precision the software float64 oracle uses, so it must reproduce it (within
// the 1-ULP truncation of the FP64 units). Loads band_f64.hex / rhs_f64.hex.
module banded_ldl_fp64 #(parameter N = 8, HB = 2,
                         parameter BAND_FILE = "../data/small/band_f64.hex",
                         parameter RHS_FILE  = "../data/small/rhs_f64.hex") (
    input  wire       clk,
    input  wire       rst_n,
    input  wire       start,
    output reg        done,
